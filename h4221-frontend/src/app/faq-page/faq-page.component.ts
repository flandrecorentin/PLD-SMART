...
export class FaqPageComponent {
  ...
  submitRecherche() {
    ...
    this.selectedqas = this.allqas.filter(
      (qa) =>
        qa.question
          .toLowerCase()
          .includes(this.rechercheForm.value.recherche!.toLowerCase()) ||
        (qa.answer != null
          ? qa.answer
              .toLowerCase()
              .includes(this.rechercheForm.value.recherche!.toLowerCase())
    ...
    this.pagenumber = 1;
    this.qas = this.selectedqas.slice(
      (this.pagenumber - 1) * 15,
      this.pagenumber * 15
    );
  }
  ...
  onClickFilter(filtre: any) {
    ...
    if (filtre == 'All') {
      ...
      document.getElementById('button' + filtre)!.style.backgroundColor =
        '#c7c7c7';
    }
    this.qas = this.selectedqas.slice(
      (this.pagenumber - 1) * 15,
  ...
  selectQuestion(qa: any) {
    ...
    document.getElementById('questiondetailspopup')!.style.visibility =
      'visible';
  }
  onClickProposer() {
    document.getElementById('popup')!.style.visibility = 'visible';
    document.getElementById('popup')!.style.opacity = '1';
    document.getElementById('proposerquestionpopup')!.style.visibility =
      'visible';
  }
  onClickClose() {
  ...
  submitQuestion() {
    ...
    this.faqService
      .askFaq({
        ...
        authorQuestion: mail,
        dateQuestion: dateText,
        categorie: this.questionForm.value.categorie,
      })
      .subscribe((datas: any) => {});
  ...
  ngOnInit() {
    this.faqService.faq().subscribe((datas: any) => {
      this.allqas = [];
